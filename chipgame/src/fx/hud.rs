...
impl FxState {
	pub fn render_ui(&mut self, g: &mut shade::Graphics, resx: &Resources) {
		...
		pool.draw(g, shade::Surface::BACK_BUFFER).unwrap();
	}
}

fn draw_sprite(cv: &mut shade::d2::DrawBuffer<UiVertex, UiUniform>, sprite: data::SpriteId, tex_size: Vec2<i32>, pos: Vec2<f32>, size: f32) {
...
fn draw_digits(cv: &mut shade::d2::DrawBuffer<UiVertex, UiUniform>, n: i32, pos: Vec2<f32>, color: u32) {
	if n < 0 {
...
fn draw_digit(cv: &mut shade::d2::DrawBuffer<UiVertex, UiUniform>, digit: Option<char>, pos: Vec2<f32>, color: u32) {
