...
fn parse_trap_linkage(index: u16, data: &[u8]) -> Result<Vec<TrapLinkage>, DatError> {
	...
	let linkages = data
		.chunks_exact(10)
		.map(|chunk| {
			let buttonx = u16::from_le_bytes([chunk[0], chunk[1]]);
			let buttony = u16::from_le_bytes([chunk[2], chunk[3]]);
			let trapx = u16::from_le_bytes([chunk[4], chunk[5]]);
			let trapy = u16::from_le_bytes([chunk[6], chunk[7]]);
			// let _unclear = u16::from_le_bytes([chunk[8], chunk[9]]);
			TrapLinkage { brown_button_x: buttonx, brown_button_y: buttony, trap_x: trapx, trap_y: trapy }
		})
		.collect();
	Ok(linkages)
}
...
fn parse_cloner_linkage(index: u16, data: &[u8]) -> Result<Vec<ClonerLinkage>, DatError> {
	...
	let linkages = data
		...
		.map(|chunk| {
			let buttonx = u16::from_le_bytes([chunk[0], chunk[1]]);
			let buttony = u16::from_le_bytes([chunk[2], chunk[3]]);
			let clonerx = u16::from_le_bytes([chunk[4], chunk[5]]);
			let clonery = u16::from_le_bytes([chunk[6], chunk[7]]);
			ClonerLinkage { red_button_x: buttonx, red_button_y: buttony, cloner_x: clonerx, cloner_y: clonery }
		})
		.collect();
	Ok(linkages)
}
